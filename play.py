...
class Valuator(object):
  def __init__(self):
    ...
    model.compile(loss="squared_hinge", optimizer="adam", metrics=["cosine_proximity"])    self.model.load_weights("model.h5")
    self.reset()
    self.memo = {}
...
def computer_minimax(s, v, depth, a, b, big=False):
  if depth >= 2 or s.board.is_game_over():
    #startval = time.time()
    val = v(s)
    #wandb.log({"Valuation Time": time.time()-startval})
    return val
...
def explore_leaves(s, v):
  ...
  #startval = time.time()
  bval = v(s)
  #wandb.log({"Valuation Time 2": startval-time.time()})
...
def computer_move(s, v):
  ...
  if len(move) == 0:
    return
  print("top 10:")
  for i,m in enumerate(move[0:10]):
    print("  ",m)
